...
export async function doGenerate(request, response) {
  ...
  try {
    ...
    if (params.limit3Rings) flags.push(`-t${params.limit3Rings}`); // Limit number of rings of length 3
    if (params.limit4Rings) flags.push(`-p${params.limit4Rings}`); // Limit number of rings of length 4
    if (params.limit5Rings) flags.push(`-f${params.limit5Rings}`); // Limit number of rings of length 5
